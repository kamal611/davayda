...
RUN CHROME_VERSION=$(google-chrome --version | grep -oP '\d+\.\d+\.\d+') &&     DRIVER_VERSION=$(curl -s "https://chromedriver.storage.googleapis.com/LATEST_RELEASE_$CHROME_VERSION") &&     wget -O /tmp/chromedriver.zip "https://chromedriver.storage.googleapis.com/$DRIVER_VERSION/chromedriver_linux64.zip" &&     unzip /tmp/chromedriver.zip -d /usr/local/bin &&     chmod +x /usr/local/bin/chromedriver &&     rm /tmp/chromedriver.zip
